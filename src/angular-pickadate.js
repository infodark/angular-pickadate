        ...
        .directive('pickadate', ['$locale', '$sce', '$compile', '$document', '$window', 'pickadateUtils',
            'pickadateI18n', 'dateFilter', function($locale, $sce, $compile, $document, $window, dateUtils, i18n, dateFilter) {
                ...
                var TEMPLATE =
                    ...
                    '<a href="" class="pickadate-prev" ng-click="changeMonth(-display)" ng-show="allowPrevMonth">' +
                    $sce.trustAsHtml(i18n.t('prev')) +
                    '</a>' +
                    '<a href="" class="pickadate-next" ng-click="changeMonth(display)" ng-show="allowNextMonth">' +
                    $sce.trustAsHtml(i18n.t('next')) +
                    '</a>' +
                    '</div>'+
                    '</div>' +
                    '<div class="pickadate-body" ng-repeat="d in dates">' +
                    ...
                    '</ul>' +
                    '<ul class="pickadate-cell">' +
                    '<li ng-repeat="day in d.dateObj" ng-click="setDate(d)" ng-class="classesFor(d)">' +
                    '{{day | date:"d"}}' +
                    '</li>' +
                    '</ul>' +
                    '</div>' +
                ...
                return {
                    ...
                    link: function(scope, element, attrs, ngModel)  {
                        ...
                        function render() {
                            ...
                            for (var i = 0; i < scope.display; i++) {
                                var monthDate = i === 0 ? initialDate : nextMonthInitialDate;
                                allDates.push({month: monthDate, dates:dateUtils.buildDates(monthDate, { weekStartsOn: weekStartsOn, noExtraRows: noExtraRows })});
                            }
                            ...
                            for (var i = 0; i < allDates.length; i++) {
                                ...
                                dates.push({month: dateFilter(month, 'MMMM yyyy'), date: date, dateObj: dateObj, classNames: classNames});
                            }
                            scope.dates = dates;
                        }
